...
// import useAutoMatch from "@/hooks/Automatch";
...
//     const {message, errors, loading} = useAutoMatch()
    
//   const selectedContest = contests.find((c) => c.id === selectedContestId);

//   const fetchSolutionLinks =async (e: React.FormEvent) => {

//     if(errors){
//       toast({
//         title: "Error",
//         description: "Error Fetching the soluton videos",
//         variant: "destructive",
//       });
//     }else{
//       toast({
...
//       <Button type="submit" onClick={fetchSolutionLinks} className="w-full hover:bg-gray-500">{loading?'Fetching the solution links':'Fetch solutions Automatically'}</Button>
//       </form>
...
const SolutionForm = ({ contests }: SolutionFormProps) => {
  ...
  const { fetchAutoMatch, message, errors, loading } = useAutoMatch();
  ...
  const fetchSolutionLinks = async (e: React.MouseEvent) => {
    ...
    try {
      ...
      toast({
        title: "Error",
        description: errors || "Error fetching solution videos",
        variant: "destructive",
      });
    }
  };
  ...
  return (
    ...
    >
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <Button
          type="button"
          onClick={fetchSolutionLinks}
          className="w-full hover:bg-gray-500"
          disabled={loading}
        >
          {loading ? "Fetching the solution links..." : "Fetch solutions Automatically"}
        </Button>
      </form>
